...
class SaeedClub(InMemoryDataset):
    ...
    def __init__(self, transform=None):
        super(SaeedClub, self).__init__('.', transform, None, None)
        ...
        idx = np.random.permutation(34)
        data.train_mask = torch.tensor([False] * 34)
        data.test_mask = torch.tensor([False] * 34)
        data.train_mask[idx[0:30]] = True
        data.test_mask[idx[30:]] = True
